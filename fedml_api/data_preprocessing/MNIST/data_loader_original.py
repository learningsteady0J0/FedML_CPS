...
def batch_data(data, batch_size):
    ...
    for i in range(0, len(data_x), batch_size):
        ...
        batched_x = batched_x.reshape(-1, 28, 28)
        batched_x = batched_x.unsqueeze(dim=1)
...
def get_data_cls(data):
    data_cls_dict = Counter({})
    ...
    for xy in data:
        y_data = xy[1]
        unq, unq_cnt = torch.unique(y_data, return_counts=True)
        dummy_cls_dict = Counter({int(unq[i]): int(unq_cnt[i]) for i in range(len(unq))})
        data_cls_dict = data_cls_dict + dummy_cls_dict

    return dict(data_cls_dict)

def load_partition_data_mnist_original(batch_size,
    ...
    local_data_cls_dict = dict()
    test_data_local_cls_dict = dict()
    ...
    for u, g in zip(users, groups):
        ...
        local_data_cls_dict[client_idx] = {}
        local_data_cls_dict[client_idx]['train'] = get_data_cls(train_batch)
        local_data_cls_dict[client_idx]['test'] = get_data_cls(test_batch)
